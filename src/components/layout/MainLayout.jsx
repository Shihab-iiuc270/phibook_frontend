...
import useGuestAuthPrompt from "../../hooks/useGuestAuthPrompt";
import GuestAuthPromptModal from "../shared/GuestAuthPromptModal";

const MainLayout = () => {
  const { user } = useAuthContext();
  const { isOpen, dismiss } = useGuestAuthPrompt({ enabled: !user });

  return (
    <div className="bg-[radial-gradient(circle_at_top_right,_#dbeafe,_#eff6ff_35%,_#f8fafc_70%)] min-h-screen pb-20 sm:pb-0">
      <Navbar />
      <GuestAuthPromptModal open={isOpen} onClose={dismiss} />
